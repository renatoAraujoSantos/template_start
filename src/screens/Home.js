...
import { NEW_PLANTS, FRIND_LIST } from '../model/Mock';
...
export default function Home({ navigation }) {
    ...
    function renderNewPlants(item, index) {
        return (
            <View style={{ alignItems: 'center', justifyContent: 'center', marginHorizontal: SIZES.base }}>
                ...
                <TouchableOpacity style={{ position: 'absolute', top: '15%', left: 7 }} onPress={() => { console.log("Focus on pressed") }} >
                    <Image
    ...
    function renderFriendsComponent() {
        if (FRIND_LIST.length == 0) {
            return (
                <View></View>
            )
        } else if (FRIND_LIST.length <= 3) {
            return (
                FRIND_LIST.map((item, index) => (
                    <View
                        key={`friend-${index}`}
                    ...
                    >
                        <Image
                            source={item.img}
                            resizeMode="cover"
                            style={{ width: 50, height: 50, borderRadius: 25, borderWidth: 3, borderColor: COLORS.primary }}
                        />
                    </View>
                ))
            )
            ...
            return (
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <Text style={{ marginLeft: 5, color: COLORS.secondary, ...FONTS.body3 }}>+{FRIND_LIST.length - 3} More</Text>
                </View>
            )
        }
    }

    function renderAcaoDeHoje() {
        return (
            <View style={{ height: "50%", backgroundColor: COLORS.lightGray }}>
                <View style={{
                    ...
                    <View style={{ marginTop: SIZES.font, marginHorizontal: SIZES.padding }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            <Text style={{ color: COLORS.secondary, ...FONTS.h2, }}>Ação de hoje</Text>
                        ...
                        <View style={{ flexDirection: 'row', height: "88%", marginTop: SIZES.base }}>
                            <View style={{ flex: 1 }}>
                                <TouchableOpacity style={{ flex: 1 }} onPress={() => { navigation.navigate("PlantDetail") }} >
                                    <Image
                                ...
                                <TouchableOpacity style={{ flex: 1, marginTop: SIZES.font }} onPress={() => { navigation.navigate("PlantDetail") }} >
                                    <Image
                            ...
                            <View style={{ flex: 1.3 }}>
                                <TouchableOpacity style={{ flex: 1, marginLeft: SIZES.font }} onPress={() => { navigation.navigate("PlantDetail") }} >
                                    <Image
            ...
            <View style={{ height: "20%", backgroundColor: COLORS.lightGray }}>
                <View style={{ flex: 1, backgroundColor: COLORS.lightGray }}>
                    <View style={{ marginTop: SIZES.radius, marginHorizontal: SIZES.padding }}>
                        ...
                        <Text style={{ color: COLORS.secondary, ...FONTS.body3, }}>{FRIND_LIST.length} total</Text>
